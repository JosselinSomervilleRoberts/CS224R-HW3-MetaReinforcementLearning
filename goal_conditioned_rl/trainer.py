...
def update_replay_buffer(
    ...
    for timestep in range(len(episode_experience)):
        ...
        if her_type == HERType.FINAL:
            ...
            final_goal = episode_experience[-1][-2]  # Get the final state from the last transition
            new_reward = env_reward_function(next_state, final_goal)  # Compute new reward
            replay_buffer.add(np.append(state, final_goal),
                              action,
                              new_reward,
                              np.append(next_state, final_goal))  # Add relabeled transition to buffer
            ...
            for _ in range(num_relabeled):
                random_timestep = np.random.randint(timestep, len(episode_experience))  # Randomly select a timestep
                random_goal = episode_experience[random_timestep][-2]  # Get the randomly selected goal
                new_reward = env_reward_function(next_state, random_goal)  # Compute new reward
                replay_buffer.add(np.append(state, random_goal),
                                  action,
                                  new_reward,
                                  np.append(next_state, random_goal))  # Add relabeled transition to buffer

        elif her_type == HERType.RANDOM:
            # random: relabel episode based on a random state from the episode
            for _ in range(num_relabeled):
                random_timestep = np.random.randint(len(episode_experience))  # Randomly select a timestep
                random_goal = episode_experience[random_timestep][-2]  # Get the randomly selected goal
                new_reward = env_reward_function(next_state, random_goal)  # Compute new reward
                replay_buffer.add(np.append(state, random_goal),
                                  action,
